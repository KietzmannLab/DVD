...
import simclr.builder, simclr.loss
...
def get_args():
    ...
    parser.add_argument('--self_supervised', type=int, default=1)
    # simclr projection_dim
...
def train_epoch(epoch, net, train_loader, optimizer, criterion, scaler, hyp):
    ...
    for batch_id, ((x_i, x_j), _) in enumerate(train_loader):


        optimizer.zero_grad()
        x_i = x_i.cuda(non_blocking=True).squeeze()  #*  [512, 1, 3, 256, 256] -> [512, 3, 256, 256]
        x_j = x_j.cuda(non_blocking=True).squeeze()  
...
if __name__ == '__main__':
    ...
    encoder, net_name = models.loader.get_network_model(hyp)
    net_name = f"simclr_{net_name}" if hyp['network']['self_supervised'] else net_name
    n_features = encoder.fc.in_features
    net= simclr.builder.SimCLR(encoder, n_features=n_features, projection_dim=hyp['network']['projection_dim'])
    ...
    criterion = simclr.loss.NT_Xent(hyp['optimizer']['batch_size'], hyp['network']['temperature'], hyp['optimizer']['world_size'])
    ...
    for epoch in range(start_epoch, n_epochs + 1):
        ...
        # Training phase | no accuracy for self-supervised learning
