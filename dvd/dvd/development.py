...
_CHROMA_PARAMS = dict(a=0.008604133954779169,
                      ...
                      min_sensitivity_threshold_age =20.37)
...
# # ---------------------------------------------------------------------------
# class DVDTransformer:
#     """Apply a develpmentasl visual diet as pre-processing piplelie for ANNs training
#     """

#     # ---------------------------------------------------------------------
#     # developmental visual diet (DVD) across 3 visual aspects: Visual acuity, Contrast & Chromatic sensitivity 
#     # ---------------------------------------------------------------------
#     @staticmethod
#     def get_early_visual_acuity(age_months: float) -> float:
#         """
#         Fitted development trajector of visual acuity maturation (Snellen-equivalent) via a dual-exponential function.
#         Based on decades of infant and child resolution data (see paper for details).
#         """
#         a, b, c, d = 18.035945052640425, 0.7933899743217134, 1.6012490927401029, 0.027054604551482078
#         return a * np.exp(-b * age_months) + c * np.exp(-d * age_months)

#     @staticmethod
#     def get_contrast_sensitivity_development( age_months: float, *,  age50: float = 4.8 * 12, n: float = 2.1633375920569247,) -> float:
#         """ Fitted development trajector of Contrast sensitivity(0–1), using a smooth, monotonic logistic fucntion.
#             Grounded in childhood CSF psychophysics (see paper for details).
#         Args:
#             age_months: Age in months.
...
#             n: fitted Steepness parameter.
#         """
#         y_max = (300 ** n) / (300**n + age50**n)  # normalizing to adult asymptote
#         return (age_months**n) / (age_months**n + age50**n) / y_max

#     @staticmethod
#     def get_chromatic_sensitivity(age_months: float) -> float:
        # """
        # itted development trajector of Contrast sensitivity(0–1), using a smooth, monotonic logistic fucntion,
        # based via decades of psychophysical data from infancy to adulthood  (see paper for details).
        # """
#         params = {
...
#             "b": 4.380740053287391e-05,
#             "alpha": 0.8807610802743646,
#             "min_age": 20.37 / 12,  #  threshold ages (months → years)
#         }
...
#         apply_color: bool = True,
#         apply_contrast: bool = True,
#         contrast_amplitude_beta: float = 0.1,         # β: sets base contrast threshold at birth
#         contrast_amplitude_lambda: float = 150,       # λ: maps age to threshold decay rate
#         apply_threshold_color: bool = False,
#         image_size: int = 224,
#         fully_random: bool = False,
#         age_months_curve: Sequence[float] | None = None,
...
#                 sensitivity = self.get_contrast_sensitivity_development(chosen_age) # Range [0, 1]
...
#                 thr = max_power * (1 - sensitivity) * 0.001 / max(math.floor(age_months / contrast_amplitude_lambda) * 2, 1) * contrast_amplitude_beta 
