...
def validate(
    ...
    for i, (images, targets) in enumerate(val_loader):
        ...
        if image_size and (images.shape[2] != image_size or images.shape[3] != image_size):
            ...
            if dvd.utils.is_main_process() and i == 0:
        ...
        if dvd.utils.is_main_process() and (i % log_freq == 0 or i + 1 == len(val_loader)):
            dvd.utils.ProgressMeter(
    ...
    if subset_class_ids is not None:
        ...
        if all_tgt.numel() == 0:
            ...
            acc1, acc5 = dvd.utils.accuracy(all_out, all_tgt, topk=(1, 5))
    else:
        acc1, acc5 = dvd.utils.accuracy(all_out, all_tgt, topk=(1, 5))
    ...
    if dvd.utils.is_main_process() and wandb_run:
        wandb_run.log({
            "val/top1": acc1.item(),
            "val/top5": acc5.item(),
            "val/loss": avg_loss,
...
def validate_in_limited(
    ...
    if subset_class_ids is not None:
        ...
        if tgt_mapped.numel() == 0:
            ...
            acc1, acc5 = dvd.utils.accuracy(all_out_subset, tgt_mapped, topk=(1, 5))
    else:
        acc1, acc5 = dvd.utils.accuracy(all_out, all_tgt, topk=(1, 5))
    ...
    if compute_per_class_acc and dvd.utils.is_main_process():
        top1_preds = all_out.argmax(dim=1)
        correct_mask = top1_preds == all_tgt
        class_correct = {}
        class_total = {}
