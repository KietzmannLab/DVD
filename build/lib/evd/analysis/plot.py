...
class ModelAccuracyPlotter:
    def __init__(self, save_dir='./results/sigma_blur_attack',
                 human_data_path='/share/klab/lzejin/lzejin/codebase/All-TNNs/evd/data/blur_degradation_human_experiment/human_accuracy.mat'):
    ...
    def plot_model_accuracy(self, new_models, dataset_name, absolute_acc=False):
        ...
        if 'face' in dataset_name:
            ...
            plt.tight_layout()
            plt.savefig(f'{self.save_dir}/human_and_models_faces_vs_sigma_blur_attack.pdf')
            ...
            y_fit = self.logistic_func(x_fit, *popt)
            self.plot_data_and_fit(self.object_df, x_fit, y_fit, cmap(0), 'Human', '^')
            ...
            plt.gca().xaxis.set_major_locator(MaxNLocator(integer=True))
            plt.tight_layout()
            plt.savefig(f'{self.save_dir}/human_and_models_objects_vs_sigma_blur_attack.pdf')
            # plt.show()
