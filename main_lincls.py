...
if __name__ == '__main__':
    ...
    # Load datasets
    train_loader, val_loader, test_loader, hyp = datasets.loader.get_dataset_loaders(hyp, ['train', 'val', 'test'])

    # Initialize network and optimizer
    net, net_name = models.loader.get_network_model(hyp)

    #* For finetuning SSL models, freeze all layers but the last fc
    ...
    if hyp['network']['pretrained']:
        net_name = f'lincls_{net_name}'
        if os.path.isfile(hyp['network']['pretrained']):
            print("=> loading checkpoint '{}'".format(hyp['network']['pretrained']))
            checkpoint = torch.load(hyp['network']['pretrained'], map_location='cuda' if torch.cuda.is_available() else 'cpu')
            ...
            print("=> loaded pre-trained model '{}'".format(hyp['network']['pretrained']))
        else:
            print("=> no checkpoint found at '{}'".format(hyp['network']['pretrained']))
