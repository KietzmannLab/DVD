...
##########################
## Blurring & Color & Contrast & Light Intensity Development Strategy
##########################
class EarlyVisualDevelopmentTransformer:
    ...
    def early_visual_acuity(self, age_months):
    ...
    def get_color_factor(self, age_months):
        ...
        def custom_average_color_mix(age_months):
            ...
            param = self.color_params["AverageRGBDevelop"]
            a, b, alpha = param["a"], param["b"], param["alpha"]
            min_age = self.color_params["min_sensitivity_threshold_ages"]
            return (T(min_age, a, b, alpha) / T(age_years, a, b, alpha)) if age_years != 0 else 0

        return custom_average_color_mix(age_months)

    def light_intensity_sensitivity_development(self, age_months):
        """Calculates normalized light intensity development as a function of age in months.

            Reference:
            Fulton, A. B., & Hansen, R. M. (2000). The Development of Scotopic Sensitivity. Investigative Ophthalmology & Visual Science, 41(6), 1588–1596.
        """
        ymax = 100
        Age50 = 19.5 # weeks
        age = age_months * (30 / 7) # Convert age from months to weeks
        n = 2.7 
        return ymax * (age ** n) / (age ** n + Age50 ** n) / 100 # Normalize to [0, 1]

    def contrast_sensitivity_development(self, age_months, age50=4.8*12, n=2.1633375920569247):
        """Reference:
    ...
    def synaptic_density_development(self,age_months):
        """
        Calculates the synaptic density percentage for a given age in months.

        Parameters:
        age_months (float): The age in months for which to calculate synaptic density.

        Returns:
        float: The synaptic density percentage.
        """
        # v1
        # peak_age = 500 / 30  # Peak synaptic density age in months
        # pruning_end_age = 4200 / 30  # End of synaptic pruning age in months
        #* Based on observation
        peak_age = 18* 12 # Peak synaptic density age in months, 18 years old
        pruning_end_age = 25 * 12  # End of synaptic pruning age in months, 25 years old

        max_density = 1.0  # Maximum percentage synaptic density
        min_density = 35 / 59  # Minimum percentage synaptic density after pruning

        
        if age_months < peak_age:
            return max_density
        elif peak_age <= age_months <= pruning_end_age:
            # Linear decrease in synaptic density
            slope = (min_density - max_density) / (pruning_end_age - peak_age)
            return max_density + slope * (age_months - peak_age)
        else:
            return min_density


    def apply_fft_transformations(self, image, age_months, cs_age50, mean_freq_factor=1, std_freq_factor=1, contrast_drop_speed_factor =1, image_size= 224, verbose=False):
        ...
        def process_image(image):
            ...
            blur_sigma = self.early_visual_acuity(age_months) * ( image_size / 224 ) # Sigma scaling with image size 
            if blur_sigma > 0:
                kernel_size = int(8 * blur_sigma) + (1 if int(8 * blur_sigma) % 2 == 0 else 0)
                # image = F.gaussian_blur(image, kernel_size=kernel_size, sigma=blur_sigma

                # Option B (if you prefer Kornia’s GPU-based blur):
            ...
            # Color
            color_factor = self.get_color_factor(age_months)
            image = self.interpolate_color_grayscale(image, color_factor)
            # image = F.adjust_saturation(image, color_factor)

            # Optionally print the original image mean if verbose is enabled
            if verbose:
            ...
            contrast_sensitivity = self.contrast_sensitivity_development(age_months, cs_age50) + 1e-10
            ...
            max_power = max([power_spectra[i].max() for i in range(3)]) # RGB  #* not right, should be the max of all channels
            threshold = max_power*0.001 * (1 - contrast_sensitivity) * contrast_drop_speed_factor # contrast_drop_speed_factor higher lower,  to control the speed of contrast drop speed, since it filters out the high frequency more
            ...
            filtered_image = torch.stack(filtered_channels, dim=1)  # Shape: [batch, 3, H, W]

            # Ensure pixel values are within the valid range [0, 1]
            filtered_image = filtered_image.clip(0, 1)
            
            # import pdb; pdb.set_trace()
            # Convert the filtered image back to a PIL image, removing the batch dimension
            return filtered_image #transforms.ToPILImage()(filtered_image.squeeze(0))  # Shape: [3, H, W]
